...
模型自适应调 PAC 依赖程度方向三轮尝试，全部失败（可学习 sigmoid 门控、监督+crossfreq 辅助损失、MI 矩阵调制 attention）。结论：只要 PAC 先验旁边留了可学习的开关或自由通路，训练就把它关掉。自适应方向不成立，转为强制方向。

重做了模型的 token 构造方式，PAC 不再改 attention 而是定义 token 本身。token = 目标频带幅值 × 按耦合强度加权、按测量优选相位对齐的慢频带相位。高频 token 旁边不保留任何原始波形通路，先验无法被绕过。频率轴换回普通 attention，PAC 只在 token 构造这一处进入模型。

预训练目标函数做了四轮迭代。从纯 crossfreq（遮住整个高频半段）出发，最终定为 cf_mixed——每个 batch 抛硬币在 crossfreq 与随机 MAE 之间二选一。实验结论：频率先验不可叠加，目标函数与物理频率位置编码同时使用反而变差，已据此把位置编码定为中性 index。

补齐基线与评测协议。BIOT、CBraMod 在我们自己的 pipeline 内跑通，取得零混淆对比数；审计并修正了两处协议错误（TUEV 模型选择指标用错导致此前一批结论撤回、TUAB 验证曲线采样过粗）。

二、讨论内容

撞车论文的处理方法。读懂对方论文真正的主旨和卖点，如果撞车部分并非核心卖点，可以拿来当参考。判断依据是主旨，不是关键词是否相同。

从生理、信号处理、数学、物理多个角度考虑故事的 novelty，看哪个角度最站得住、最好讲。

预训练算力改用 AMD 卡。

三、后续计划

短期先验证目标函数是否适配新的 token 构造方式。

验证新结构的优势是否稳定。

修改代码适配 AMD 卡，准备启动大规模预训练。

> **会后进展（截至 2026-07-31，供下次会议参考）。** 三条计划都已执行完毕，结果如下：
> - **新结构的优势成立且跨数据集**（§13.43-J）：TUEV +0.081、Sleep-EDF +0.052、TUSZ 打平、
>   TUAB 未测。三个参数对齐的匹配对照（`uniform`/`concat`/`magnitude`）全部与基线打平，
>   排除了"乘法结构本身""暴露相位特征""按强度加权"三种替代解释。同 pipeline 零混淆对比下
>   TUEV 第一：1.64M 参数领先 CBraMod-scratch（4.88M）0.048。
> - **目标函数已消融完毕，但结论是负面的**（§13.44a / §13.45）：会上"cf_mixed 为记录方案"
>   这一条**已被推翻**。当时引用的收益倍数用了配置错误的对照（`random`+index）；与配置正确的
>   `random`+hz 相比 `cf_mixed` 本就只是持平。会后补了九个目标函数 × 三个数据集，minimax
>   判据下 `cf_mixed` 从未赢过标准随机 MAE（worst-case −0.014），方案改为 `random` MAE。
>   同样被推翻的还有会上那条"频率先验不可叠加、故位置编码取 index"——六个 BandPE 对比方向
>   全不一致，`band_pe` 保持 index 但理由改为"无证据支持改动"（§13.44a）。
>   更要紧的是：**在两个监督能正常工作的数据集上，最优预训练方案依然不如不预训练**，唯一的
>   胜利来自监督直接塌掉的 CHB-MIT。当前规模下预训练是救场手段，不是通用先验。
> - **AMD 卡迁移尚未开始。**
> - 由此，论文主张收缩为单一条：强制的、规范不变的 PAC token 构造。上次会议提的
>   "容易被误解为 self-attention 的小变种"已从结构上解决——PAC 不再改 attention。
